...
if sgn == 0
   % In this case, we have a polynomial of order <= 4.
   % So, computing quartic coeffcients using the following:
   %
   % (k + 1)^2*(2*k + 1)^2 = 4*k^4 + 12*k^3 + 13*k^2 + 6*k + 1
   %
   % (2*k + 1)^2*(2*k - 1)^2 = 16*k^4 - 8*k^2 + 1
   %
   % (k + 1)^2*(2*k - 1)^2 = 4*k^4 + 4*k^3 - 3*k^2 - 2*k + 1
   %
   a0 = ksi1 + ksi2 + ksi3;
   a1 = ksi1*6 + ksi3*(-2);
   a2 = ksi1*13 + ksi2*(-8) + ksi3*(-3);
   a3 = ksi1*12 + ksi3*4;
   a4 = ksi1*4 + ksi2*16 + ksi3*4;
   a = [a0, a1, a2, a3, a4];
  ...
  if sq_zeta(1) > ALMOST_ZERO && sq_zeta(2) > ALMOST_ZERO && sq_zeta(3) > ALMOST_ZERO
    ...
    % (k + 1)^2*(2*k - 1)^2 = 4*k^4 + 4*k^3 - 3*k^2 - 2*k + 1
    %
    % (k + 1)^2*(2*k - 1)^2*(2*k + 1)^2 = 16 k^6 + 32 k^5 + 8 k^4 - 16 k^3 - 7 k^2 + 2 k + 1
    %  
  ...
  elseif sq_zeta(1) < ALMOST_ZERO && sq_zeta(2) > ALMOST_ZERO && sq_zeta(3) > ALMOST_ZERO
    ...
    % So, using the following expansions to get the coefficients:
    %
    % (2*k + 1)^2 = 4*k^2 + 4*k + 1 (ξ2)
    %
    % (k + 1)^2 = k^2 + 2*k + 1 (ξ3)
    %
    % (k + 1)^2*(2*k + 1)^2 = 4*k^4 + 12*k^3 + 13*k^2 + 6*k +1 (sgn)
    %  
  ...
  elseif sq_zeta(1) > ALMOST_ZERO && sq_zeta(2) < ALMOST_ZERO && sq_zeta(3) > ALMOST_ZERO
    ...
    % So, using the following expansions to get the coefficients:
    %
    % (2*k + 1)^2 = 4*k^2 + 4*k + 1 (ξ1)
    %
    % (2*k - 1)^2 = 4*k^2 - 4*k + 1 (ξ3)
    %
    % (2*k - 1)^2*(2*k + 1)^2 = 16*k^4 - 8*k^2 +1 (sgn)
    %
    a0 = sgn*1 - ksi1 - ksi3;
    a1 = 0 - ksi1*4 - ksi3*(-4);
